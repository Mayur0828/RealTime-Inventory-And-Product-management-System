...
def calculate_monthly_business_amount(orders_df):
    # Convert ActualOrderDeliveredDate to datetime format
    ...
    filtered_orders = orders_df[orders_df['ActualOrderDeliveredDate'] >= one_year_ago]
    
    # Group by month and sum the Amount for each month
    filtered_orders['YearMonth'] = filtered_orders['ActualOrderDeliveredDate'].dt.to_period('M')
    monthly_business_amount = filtered_orders.groupby('YearMonth')['Amount'].sum().reset_index()
    monthly_business_amount['YearMonth'] = monthly_business_amount['YearMonth'].dt.to_timestamp()

    # Create a date range for the past 12 months
    date_range = pd.date_range(start=one_year_ago, end=datetime.now(), freq='MS')

    # Reindex the DataFrame to include all months in the date range
    monthly_business_amount = monthly_business_amount.set_index('YearMonth').reindex(date_range).fillna(0).rename_axis('YearMonth').reset_index()

    return monthly_business_amount
    ...
    if session['LoggedIn']==True and (session['Designation']=="Admin"):
        ...
        if delivery_person:
            ...
            if delivery_person_count > 0:
                ...
                # If not referenced, proceed with deletion
                employeeDelivery=Users.query.filter_by(User_ID=DeliveryPersonID).first()
                db.session.delete(employeeDelivery)
...
def update_plots(pathname):
    ...
    monthly_business_amount_df = calculate_monthly_business_amount(order_df)
    business_amount_plot = px.line(monthly_business_amount_df, x='YearMonth', y='Amount', title='Monthly Business Amount (Past Year)',
                                   labels={'YearMonth': 'Month', 'Amount': 'Total Business Amount'})
